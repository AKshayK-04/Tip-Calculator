...
//if there is only one person then we need not to display each.
    ...
    /*calculating the tip by multiplying total-bill and type of service;
     then dividing it by number of persons.*/
            ...
            let total_tip = (amount * (service)/100) / persons;
            total_tip = total_tip.toFixed(2);
 
            //finally displaying the tip value
            document.querySelector('.tip').style.display = 'block';
            document.querySelector('#total').innerHTML = total_tip;


		    //finally displaying the total bill value
			let total_bill = (amount / persons)+(amount * (service)/100) / persons;
            total_bill = total_bill.toFixed(2);
			document.querySelector('.tip').style.display = 'block';
            document.querySelector('#totalbill').innerHTML = total_bill;
